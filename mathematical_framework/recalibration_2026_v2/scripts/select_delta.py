...
def main() -> int:
    ...
    frozen_seeds = {str(item["seed"]) for item in specs}
    condition_by_seed = {str(item["seed"]): str(item["condition"]) for item in specs}

    grouped: dict[float, list[dict[str, Any]]] = defaultdict(list)
    seen_units: set[tuple[float, str, int]] = set()
    input_records = []
    expected_evaluation_n = int(config["partition"]["evaluation_n"])
    for raw_path in args.harm_csv:
        path = Path(raw_path).resolve()
        harm_summary_path = path.with_suffix(".summary.json")
        if not harm_summary_path.is_file():
            raise SystemExit(f"Missing V2 harm summary: {harm_summary_path}")
        harm_summary = load_json(harm_summary_path)
        if int(harm_summary.get("evaluation_n_per_block", -1)) != expected_evaluation_n:
            raise SystemExit(f"Non-V2 evaluation size in {harm_summary_path}")
        with path.open(newline="", encoding="utf-8") as handle:
            rows = list(csv.DictReader(handle))
        input_records.append({
            "path": str(path),
            "sha256": sha256_file(path),
            "summary_path": str(harm_summary_path),
            "summary_sha256": sha256_file(harm_summary_path),
        })
        for row in rows:
    ...
    for delta in frozen_grid:
        ...
        for condition in sorted(set(condition_by_seed.values())):
            condition_all = [
                row for row in rows
                if condition_by_seed[str(row["trajectory_seed"])] == condition
            ]
            condition_info = [row for row in condition_all if int(row["informative"]) == 1]
            condition_rows.append({
                "delta_W": delta,
                "condition": condition,
                "trajectory_count": len({row["trajectory_seed"] for row in condition_all}),
                "informative_events": len(condition_info),
                "harm_events": sum(int(row["H"]) for row in condition_info),
                "nonzero_commits": sum(
                    int(row["committed_nonzero"]) for row in condition_info
                ),
                "update_coverage": (
                    sum(int(row["committed_nonzero"]) for row in condition_info)
                    / len(condition_info) if condition_info else 0.0
                ),
            })
    ...
    condition_table_path = output / "delta_calibration_by_condition.csv"
    write_csv(condition_table_path, condition_rows)
    result = {
        "version": "delta_calibration_selection_v2",
        "status": status,
        ...
        "input_lock": {"path": str(input_lock_path), "sha256": sha256_file(input_lock_path)},
        ...
        "condition_summary_table": {
            "path": str(condition_table_path),
            "sha256": sha256_file(condition_table_path),
        },
    }
