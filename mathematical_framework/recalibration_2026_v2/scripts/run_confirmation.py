...
def main() -> int:
    ...
    if selection.get("status") != "positive_delta_selected":
        raise SystemExit("V2 confirmation requires a positive frozen delta")
    if selection.get("protocol", {}).get("sha256") != sha256_file(protocol_path):
        raise SystemExit("Selected delta belongs to a different protocol")
    ...
    write_json(output / "confirmation_plan.json", {
        "version": "v2_confirmation_plan_v1",
