...
def draw_base_plate(
    ...
    for ch in plate:
        ...
        char_metrics.append((bbox[2] - bbox[0], bbox[3] - bbox[1]))
    ...
    for idx, ch in enumerate(plate):
        ...
        local = np.array(mask.crop((max(0, x - 3), max(0, y - 3), min(width, x + w + 6), min(height, y + h + 8))))
        ys, xs = np.where(local > 0)
        if len(xs) > 0:
            bx0 = max(0, x - 3) + int(xs.min())
            by0 = max(0, y - 3) + int(ys.min())
            bx1 = max(0, x - 3) + int(xs.max()) + 1
            by1 = max(0, y - 3) + int(ys.max()) + 1
        else:
...
def create_dataset(args: argparse.Namespace) -> None:
    ...
    (out / "dataset_config.json").write_text(json.dumps(config, indent=2, ensure_ascii=False), encoding="utf-8")
